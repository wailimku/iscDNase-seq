...
%You will need to set the path. For example, '/yourpath/iscDNase-seq/barcode_B_sc/';
%==================================================
...
a = readtable(strcat(char(path0),'/pre_data/barcode_96.txt'),'ReadVariableNames',0);
...
%==================================================
command9 = 'wc -l ./barcode_B_sc/*/*.uniq|awk ''{print $1"\t"$2}''>wc_uniq2.txt';
...
fp = fopen(strcat(char(path),'/script_cp_selfile'),'w');
for i= 1:max(size(q))
...
%============================================
%run "mkdir selbed" in path0
%run "sh script_cp_selfile" in path0
%==================================================
